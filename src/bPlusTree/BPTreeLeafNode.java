...
public class BPTreeLeafNode<T extends Comparable<T>> extends BPTreeNode<T> implements Serializable{
	...
	public PushUp<T> insert(T key, Ref recordReference, BPTreeInnerNode<T> parent, int ptr)
	{
		...
		if(result[1] == 1) {	//the key is duplicated
			if(!(this.records[result[0]] instanceof OverflowPage)) {
				...
				OverflowPage page = (OverflowPage) this.records[result[0]];
	...
	{
		if(records[index] instanceof OverflowPage) {
			OverflowPage page = (OverflowPage) records[index];
			page.setNext(null);
			page = null;
		}
